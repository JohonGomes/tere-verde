...
async function run() {
    ...
    `);
        ...
        const [likesDb] = await connection.query("SELECT * FROM user_likes WHERE target_name = 'Pedra do Sino'");
        console.log("📊 Curtidas registradas no MySQL para 'Pedra do Sino':", likesDb.length);
        console.log(likesDb);
        const [commentsDb] = await connection.query("SELECT * FROM comments WHERE target_name = 'Pedra do Sino'");
