  ...
  try {
    ...
    if (req.headers.authorization && req.headers.authorization.startsWith("Bearer")) {
      ...
      try {
        ...
        const [userRows]: any = await pool.execute("SELECT role FROM users WHERE id = ?", [decoded.id]);
...
export async function moderateComment(req: AuthenticatedRequest, res: Response) {
  try {
    const { id } = req.params;
...
export async function getReviews(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    const [rows] = await pool.execute(
      "SELECT id, user_id as userId, user_name as userName, target_id as targetId, target_type as targetType, rating, comment, created_at as createdAt FROM reviews WHERE target_id = ? AND target_type = ? ORDER BY created_at DESC",
      [targetId as string, targetType as string]
    );
...
export async function addReview(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    const [existingReviews]: any = await pool.execute(
      "SELECT id FROM reviews WHERE user_id = ? AND target_id = ?",
      [userId, targetId]
    );
    ...
    const reviewId = `review-${crypto.randomUUID().slice(0, 8)}`;
    const userName = req.user!.name;

    // 2. Inserir avaliação
    await pool.execute(
      "INSERT INTO reviews (id, user_id, user_name, target_id, target_type, rating, comment) VALUES (?, ?, ?, ?, ?, ?, ?)",
      [reviewId, userId, userName, targetId, targetType, ratingVal, comment || ""]
    );

    // 3. Recalcular a média e contagem de avaliações para o destino
    const [statsRows]: any = await pool.execute(
      "SELECT COUNT(id) as contagem, AVG(rating) as media FROM reviews WHERE target_id = ?",
      [targetId]
    );
    ...
    if (targetType === "Restaurante") {
      await pool.execute(
        "UPDATE restaurants SET nota_media = ?, avaliacoes_contagem = ? WHERE id = ?",
        [media, contagem, targetId]
      );
    } else if (targetType === "Hospedagem") {
      await pool.execute(
        "UPDATE lodgings SET nota_media = ?, avaliacoes_contagem = ? WHERE id = ?",
        [media, contagem, targetId]
      );
    }
