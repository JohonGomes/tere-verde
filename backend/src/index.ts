  ...
  try {
    ...
    try {
      ...
      const [columns]: any = await connection.query(
        "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'parks'",
        [process.env.DB_NAME || "tere_verde"]
      );
      ...
      const [commentColumns]: any = await connection.query(
        "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'comments'",
        [process.env.DB_NAME || "tere_verde"]
      );
      const commentColNames = commentColumns.map((c: any) => c.COLUMN_NAME.toLowerCase());
      ...
      for (const park of defaultParksData) {
        const [existing]: any = await connection.query("SELECT id FROM parks WHERE id = ?", [park.id]);
        if (existing.length === 0) {
          ...
          await connection.query(
            "UPDATE parks SET video = IFNULL(video, ?), principais_trilhas = IFNULL(principais_trilhas, ?), cachoeiras = IFNULL(cachoeiras, ?), galeria_fotos = IFNULL(galeria_fotos, ?), como_chegar = IFNULL(como_chegar, ?) WHERE id = ?",
            [
              park.video,
              JSON.stringify(park.principais_trilhas),
              JSON.stringify(park.cachoeiras),
